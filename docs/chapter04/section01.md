...
4. **使用Git推送代码**
   - 在Solo Trae Web Code的终端中添加远程仓库：
     ```bash
     git remote add cnb https://cnb.cool/slowdown87/tutorial-project.git
     ```
   - 更新远程仓库URL，在URL中包含用户名和访问令牌：
     ```bash
     git remote set-url cnb https://cnb:<访问令牌>@cnb.cool/slowdown87/tutorial-project.git
     ```
     注意：将 `<访问令牌>` 替换为你刚才创建的令牌
   - 推送代码到cnb.cool：
     ```bash
     git push cnb main
     ```
...
### Q: 如何创建访问令牌？
**A:** 进入个人设置页面，点击「访问令牌」或「Access Token」，然后点击「创建新令牌」按钮，输入令牌名称和权限，点击「创建」按钮即可。记得复制生成的令牌，只显示一次！
